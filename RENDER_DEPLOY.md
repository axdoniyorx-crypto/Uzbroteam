...
- `CHANNEL_ID` — bot foydalanadigan ichki saqlash kanali (inline
  natijalar uchun media faylларни vaqtincha joylashtirishda ishlatiladi).
  Eslatma: bu majburiy obuna bilan bog'liq emas — majburiy obunaga
  yo'naltirish endi butunlay o'chirilgan, foydalanuvchi hech qanday
  kanalga obuna bo'lishga majbur qilinmaydi.
- `COBALT_API_URL`, `COBALT_API_KEY` — Cobalt media API. `COBALT_API_URL`
  o'rnatilmasa `COBALT_URL` ga, u ham bo'lmasa standart ochiq instansiya
  (`https://uzbroteam.onrender.com/`) ga tushadi (https va public IP talab
  qilinadi, kod tekshiradi). `COBALT_API_KEY` ixtiyoriy — kalit
  ko'rsatilmasa so'rov autentifikatsiyasiz yuboriladi.
