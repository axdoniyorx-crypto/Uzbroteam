...
_ADMIN_ACCESS_REQUIRED = "Administrator huquqi talab qilinadi."
...
class Admin(StatesGroup):
    ...
    write_welcome_message = State()


_WELCOME_MESSAGE_MAX_LENGTH = 4000
...
async def _cleanup_downloads_once() -> tuple[str, bool]:
    """Returns (status_text, did_finish)."""
    queue_snapshot = get_download_queue().load_snapshot()
    if queue_snapshot.active_jobs > 0 or queue_snapshot.queued_jobs > 0:
        return bm.downloads_cleanup_blocked(queue_snapshot.active_jobs, queue_snapshot.queued_jobs), False
    if not os.path.exists(OUTPUT_DIR):
        return f"'{OUTPUT_DIR}' papkasi mavjud emas.", False
...
async def admin_cleanup_downloads(call: types.CallbackQuery):
    if not await _ensure_admin_callback(call):
        return
    status_text, did_finish = await _cleanup_downloads_once()
    await call.answer("Tozalash yakunlandi" if did_finish else "Tozalash o'tkazib yuborildi")
    queue_snapshot = get_download_queue().load_snapshot()
    await call.message.edit_text(
...
async def cancel_action(call: types.CallbackQuery, state: FSMContext):
    ...
    if current_state is None:
        await call.answer("Bekor qilish uchun hech nima yo'q")
        return
...
@router.callback_query(F.data == 'admin_welcome_message')
async def admin_welcome_message_menu(call: types.CallbackQuery, state: FSMContext):
    if not await _ensure_admin_callback(call):
        return
    ...
    custom_text = await db.get_welcome_message()
    is_custom = bool(custom_text)
    current_text = custom_text or bm.default_welcome_message()

    await call.message.edit_text(
        bm_admin.welcome_message_settings_menu(current_text, is_custom),
        reply_markup=kb.welcome_message_settings_keyboard(is_custom),
        parse_mode="HTML",
    )


@router.callback_query(F.data == 'admin_welcome_message_edit')
async def admin_welcome_message_edit_prompt(call: types.CallbackQuery, state: FSMContext):
    if not await _ensure_admin_callback(call):
        return

    await call.answer()
    await call.message.edit_text(
        bm_admin.welcome_message_prompt(),
        reply_markup=kb.welcome_message_cancel_keyboard(),
    )
    await state.set_state(Admin.write_welcome_message)


@router.message(Admin.write_welcome_message)
async def admin_collect_welcome_message(message: types.Message, state: FSMContext):
    ...
    if not new_text:
        await message.reply(bm_admin.welcome_message_invalid())
        return
    if len(new_text) > _WELCOME_MESSAGE_MAX_LENGTH:
        await message.reply(bm_admin.welcome_message_too_long(_WELCOME_MESSAGE_MAX_LENGTH))
        return

    await db.set_welcome_message(new_text)
    await state.clear()

    await message.answer(
        bm_admin.welcome_message_updated(),
        reply_markup=kb.welcome_message_settings_keyboard(is_custom=True),
        parse_mode="HTML",
    )


@router.callback_query(F.data == 'admin_welcome_message_reset')
async def admin_welcome_message_reset(call: types.CallbackQuery, state: FSMContext):
    if not await _ensure_admin_callback(call):
        return

    await db.reset_welcome_message()
    await call.answer(bm_admin.welcome_message_reset())

    current_text = bm.default_welcome_message()
    await call.message.edit_text(
        bm_admin.welcome_message_settings_menu(current_text, is_custom=False),
        reply_markup=kb.welcome_message_settings_keyboard(is_custom=False),
        parse_mode="HTML",
    )
...
async def mailing_target_callback(call: types.CallbackQuery, state: FSMContext):
    ...
    if target_audience == "dm":
        audience_name = "Faqat shaxsiy chatlar"
