...
from services.platforms.audio_callback_tokens import register_audio_url, resolve_audio_url
...
async def process_instagram_video(
    ...
    audio_callback_data = f"audio:inst:{register_audio_url(original_url)}"
...
async def download_instagram_audio_callback(call: types.CallbackQuery):
    ...
    await call.answer()
    token = call.data.replace("audio:inst:", "")
    original_url = resolve_audio_url(token)
    if not original_url:
        await safe_edit_text(call.message, bm.audio_button_expired())
        return
