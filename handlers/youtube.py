...
def search_youtube_tracks(query: str, limit: int = 10):
    return youtube_media_service.search_youtube_tracks(query, limit=limit)
...
async def _get_youtube_video_with_timeout(url: str):
    ...
    # yt-dlp info extraction failed (often YouTube's bot-check on the
    # server IP). Try resolving minimal metadata through Cobalt instead
    # of giving up outright.
    try:
        return await youtube_media_service.get_youtube_video_via_cobalt(url)
    except Exception as exc:
        logging.debug("Cobalt metadata fallback failed: url=%s error=%s", url, exc)
        return None
