...
async def _maybe_show_optional_channel_promo(message: types.Message) -> None:
    """Show a lightweight, non-blocking promo for the admin-configured
    optional (non-mandatory) channel(s), if the feature is enabled and at
    least one optional channel exists. Joining is never required."""
    try:
        if not await user_mod.db.is_optional_channel_enabled():
            return
        channels = await user_mod.db.get_all_channels(subscription_type="optional")
        if not channels:
            return
        from keyboards.optional_channel import build_optional_channel_keyboard
        user_language = (await user_mod.db.user_settings(message.from_user.id)).get("language", "uz")
        promo_text = (
            "📣 Bizning qo'shimcha kanalimizga a'zo bo'ling!"
            if user_language == "uz"
        ...
        logging.debug("Skipping optional channel promo: %s", exc)
...
async def send_welcome(message: types.Message):
    ...
    user_language = "uz"
    try:
        settings = await user_mod.db.user_settings(message.from_user.id)
        user_language = (settings or {}).get("language", "uz")
    except Exception:
        pass
    await message.reply(
        await bm.welcome_message(language=user_language),
        reply_markup=kb.start_keyboard(bot_username, ref_user_id=message.from_user.id),
        parse_mode="HTML",
    )

    if message.chat.type == ChatType.PRIVATE:
        await _maybe_show_optional_channel_promo(message)
...
async def send_help(message: types.Message):
    ...
    try:
        ...
        pass
    await message.reply(
        bm.help_message(bot_username, language=user_language),
        reply_markup=kb.start_keyboard(bot_username),
        parse_mode="HTML",
    )


async def show_help_callback(call: types.CallbackQuery):
    bot_username = await get_bot_username(user_mod.bot)
    user_language = "uz"
    try:
        settings = await user_mod.db.user_settings(call.from_user.id)
        user_language = (settings or {}).get("language", "uz")
    except Exception:
        pass
    text = bm.help_message(bot_username, language=user_language)
    try:
