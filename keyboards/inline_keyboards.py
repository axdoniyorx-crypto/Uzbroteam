...
def start_keyboard(bot_username: str | None = None, ref_user_id: int | None = None) -> InlineKeyboardMarkup:
    """Start command keyboard with inline, settings, help, share and group add buttons."""
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                ...
                InlineKeyboardButton(text="🌐 Til / Language", callback_data="show_language_choice"),
            ],
            [
...
def welcome_message_cancel_keyboard() -> InlineKeyboardMarkup:
    """Keyboard shown while waiting for the admin to type a new welcome message."""
    return InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text="❌ Bekor qilish", callback_data="admin_welcome_message")]]
    )
...
def music_search_results_keyboard(session_id: str, count: int) -> InlineKeyboardMarkup:
    """Numbered result-picker keyboard for text-based music search (1-10 in
    rows of 5), plus a cancel button, matching the familiar
    number-grid pattern from popular music-search bots."""
    number_buttons = [
        InlineKeyboardButton(text=str(i), callback_data=f"music_pick:{session_id}:{i}")
        for i in range(1, count + 1)
    ]
    rows = [number_buttons[i:i + 5] for i in range(0, len(number_buttons), 5)]
    rows.append([InlineKeyboardButton(text="❌ Bekor qilish", callback_data=f"music_cancel:{session_id}")])
    return InlineKeyboardMarkup(inline_keyboard=rows)


def shazam_result_keyboard(session_id: str) -> InlineKeyboardMarkup:
    """Keyboard shown after a successful Shazam match: download the full
    track via the existing YouTube pipeline, or dismiss the result."""
    rows = [
        [InlineKeyboardButton(text="⬇️ To'liq qo'shiqni yuklash", callback_data=f"shazam_download:{session_id}")],
        [InlineKeyboardButton(text="❌ Bekor qilish", callback_data=f"shazam_cancel:{session_id}")],
    ]
    return InlineKeyboardMarkup(inline_keyboard=rows)
