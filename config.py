...
# COBALT_API_URL: the base URL of the Cobalt downloader API this bot proxies
# through. Resolution order: COBALT_API_URL -> COBALT_URL (legacy alias) ->
# a default public Cobalt instance. This mirrors:
#   COBALT_API_URL = os.getenv("COBALT_API_URL", os.getenv("COBALT_URL", "https://uzbroteam.onrender.com/"))
# Both variables are configured purely through environment variables (.env
# locally, Render Environment Variables in production) — nothing here is
# hardcoded to a secret value.
COBALT_API_URL = (
    _read_env("COBALT_API_URL", aliases=("COBALT_URL",)) or "https://uzbroteam.onrender.com/"
)
...
# COBALT_API_KEY is optional — the default Cobalt instance above does not
# require one. If your Cobalt deployment does require an API key, set
# COBALT_API_KEY in the environment and it will be sent as an
# "Authorization: Api-Key <value>" header (see utils/cobalt_client.py).
COBALT_API_KEY = _read_env("COBALT_API_KEY") or ""
...
# Note: BOT_POLLING_TASKS_CONCURRENCY_LIMIT was used by long-polling mode.
# The bot now runs in Flask webhook mode (see main.py), so this setting is
# unused; kept only in case polling mode is restored later.
...
# SHAZAM_API_URL: base URL of the standalone Shazam recognition microservice
# (see shazam_service/ — a separate FastAPI app deployed as its own Render
# Web Service, mirroring how COBALT_API_URL above proxies through Cobalt).
SHAZAM_API_URL = _validate_custom_api_url(
    _read_env("SHAZAM_API_URL") or "", "SHAZAM_API_URL"
) or None
# SHAZAM_API_KEY is sent as the "X-API-Key" request header. Optional: only
# required if the deployed Shazam service was configured with an API_KEY.
SHAZAM_API_KEY = _read_env("SHAZAM_API_KEY") or ""
SHAZAM_RECOGNIZE_TIMEOUT_SECONDS = _read_float_env("SHAZAM_RECOGNIZE_TIMEOUT_SECONDS") or 55.0
# Telegram's Bot API only allows downloading files up to 20 MB via getFile,
# regardless of MAX_FILE_SIZE above (which governs bot->user uploads, not
# user->bot downloads). This is a hard Telegram limit, not ours to raise.
TELEGRAM_FILE_DOWNLOAD_LIMIT = 20 * 1024 * 1024

HTTP_HEALTH_HOST = _read_env("HTTP_HEALTH_HOST") or "0.0.0.0"
HTTP_HEALTH_PORT = _read_int_env("HTTP_HEALTH_PORT") or 8080

# --- Webhook mode (Flask) ---
# WEBHOOK_URL: public base URL of this service, e.g. https://my-bot.onrender.com
# The bot will register https://my-bot.onrender.com/webhook/<WEBHOOK_SECRET_PATH> with Telegram.
WEBHOOK_URL = _read_env("WEBHOOK_URL")
WEBHOOK_SECRET_PATH = _read_env("WEBHOOK_SECRET_PATH") or BOT_TOKEN.split(":")[-1]
WEBHOOK_SECRET_TOKEN = _read_env("WEBHOOK_SECRET_TOKEN")
