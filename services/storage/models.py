...
# Key used in app_settings to globally enable/disable the mandatory
# subscription check without touching individual channel rows.
MANDATORY_SUBSCRIPTION_TOGGLE_KEY = "mandatory_subscription_enabled"
...
class AppSetting(Base):
    """Simple global key/value store for bot-wide toggles (e.g. mandatory
    subscription on/off) that don't warrant their own dedicated table."""
...
class Channel(Base):
    ...
    subscription_type = Column(Text, nullable=False, default="mandatory")
...
class ChannelSubscription(Base):
    ...
    __table_args__ = (
        Index("ix_channel_subscriptions_user_id", "user_id"),
        Index("ix_channel_subscriptions_channel_id", "channel_id"),
    ...
    is_subscribed = Column(Boolean, nullable=False, default=False, server_default=sa.text("false"))
...
class BroadcastMessage(Base):
    ...
    target_type = Column(Text, nullable=False, default="all")
    status = Column(Text, nullable=False, default="draft")
    scheduled_at = Column(TIMESTAMP(timezone=True), nullable=True)
    sent_count = Column(BigInteger, default=0, server_default=sa.text("0"))
    failed_count = Column(BigInteger, default=0, server_default=sa.text("0"))
    created_at = Column(TIMESTAMP(timezone=True), server_default=func.now())
