...
class UserRepositoryMixin:
    ...
    async def search_users(self, query: str, limit: int = 10) -> list[Any]:
        """Search users by numeric user_id, username, or display name."""
        async with self.SessionLocal() as session:
            query_stripped = query.strip().lstrip("@")
            conditions = [
                User.user_username.ilike(f"%{query_stripped}%"),
                User.user_name.ilike(f"%{query_stripped}%"),
            ]
            if query_stripped.lstrip("-").isdigit():
                conditions.append(User.user_id == int(query_stripped))
            result = await session.execute(
                select(
                    User.user_id,
                    User.user_name,
    ...
    async def get_banned_users(self, limit: int = 50) -> list[Any]:
        async with self.SessionLocal() as session:
            result = await session.execute(
                select(
                    User.user_id,
