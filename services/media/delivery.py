...
async def deliver_track_as_audio(
    *,
    ...
    status_uploading_text: str,
    too_large_text: str,
    on_missing_text_fn: Callable[[], Awaitable[None]],
    caption_fn: Callable[[Any], str],
    send_chat_action_fn: Callable[..., Awaitable[None]],
    safe_edit_text_fn: Callable[..., Awaitable[Any]],
    get_bot_avatar_thumbnail_fn: Callable[..., Awaitable[Any]],
    remove_file_fn: Callable[[Optional[str]], Awaitable[None]],
    retry_async_operation_fn: Callable[..., Awaitable[Any]],
    get_youtube_video_fn: Callable[[str], Any],
    download_mp3_fn: Callable[..., Awaitable[Any]],
) -> Optional[types.Message]:
    """Shared "resolve a YouTube webpage_url into an MP3 and send it" pipeline.

    Used by both the text-based /music search (music_search.py) and the
    Shazam recognition flow: both end up with a YouTube track dict that
    needs the exact same cache-lookup -> download -> metadata -> upload ->
    cache-store sequence, so this is that sequence factored out once.

    ``unique_id`` should be a value stable per source (video id, or a
    fallback like a session/file id) — it's only used to name the temp
    download file, not for caching (caching uses the webpage_url itself).
    """
    webpage_url = track.get("webpage_url")
    if not webpage_url:
        await on_missing_text_fn()
        return None

    cache_key = build_audio_cache_key(webpage_url)
    full_track: dict | None = None

    async def _send_cached(file_id: str):
        await safe_edit_text_fn(message, status_uploading_text)
        await send_chat_action_fn(bot, message.chat.id, "upload_audio", business_id)
        return await message.answer_audio(
            audio=file_id,
            caption=caption_fn(None),
            parse_mode="HTML",
        )

    async def _fetch_metadata() -> bool:
        nonlocal full_track
        full_track = await asyncio.to_thread(get_youtube_video_fn, webpage_url)
        if not full_track:
            full_track = {
    ...
    async def _download_audio():
        video_id = (full_track or {}).get("id") or track.get("id") or unique_id
        base_name = f"{video_id}_track_audio"
        return await retry_async_operation_fn(
            lambda: download_mp3_fn(
                webpage_url,
                base_name,
                "track_audio_mp3",
    ...
    async def _prepare_metadata(path: str):
        return await prepare_mp3_metadata(path, full_track or track)
    ...
    async def _send_downloaded(path: str, prepared_metadata):
        ...
        thumbnail = (
            ...
            else await get_bot_avatar_thumbnail_fn(bot)
        )
        title = (full_track or track).get("title")
        performer = (full_track or track).get("uploader") or (full_track or track).get("artist")
        return await send_audio_with_thumbnail(
            ...
            bot_url=bot_url,
            duration=(full_track or track).get("duration"),
    ...
    result = await run_audio_flow(
        ...
        cleanup_path=remove_file_fn,
    )
    return result.sent_message if result else None
