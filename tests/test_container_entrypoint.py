...
def test_resolve_command_uses_default_when_empty():
    assert container_entrypoint._resolve_command([]) == container_entrypoint.DEFAULT_COMMAND
    # The default command is run through a shell so that ${PORT} (injected by
    # Render/most PaaS providers) is honored, falling back to 8080 otherwise.
    assert container_entrypoint.DEFAULT_COMMAND[0] == "sh"
    assert container_entrypoint.DEFAULT_COMMAND[1] == "-c"
    assert "gunicorn" in container_entrypoint.DEFAULT_COMMAND[-1]
    assert "main:flask_app" in container_entrypoint.DEFAULT_COMMAND[-1]
    assert "${PORT:-8080}" in container_entrypoint.DEFAULT_COMMAND[-1]
