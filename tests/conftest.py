...
# Prevent main.py from spinning up a real aiogram Bot / event-loop thread
# (which calls Telegram's API) just because some test imports it.
os.environ.setdefault("BOT_AUTOSTART", "0")

# Dummy values so config.py's required-env-var checks pass under pytest.
# No real Telegram API or database is contacted during unit tests.
os.environ.setdefault("BOT_TOKEN", "123456:TEST-dummy-token-for-unit-tests-only")
