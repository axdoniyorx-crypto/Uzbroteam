...
async def fetch_cobalt_data(
    ...
    *,
    ...
    # COBALT_API_KEY is optional: many Cobalt instances (including public
    # ones) do not require an API key. Only attach the Authorization header
    # when a key is actually configured; otherwise proceed unauthenticated
    # instead of failing the request outright.
    auth_header = _build_auth_header(api_key)
    if not auth_header:
        logging.debug("No COBALT_API_KEY configured for %s; sending unauthenticated request", source)
